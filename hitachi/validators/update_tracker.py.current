...
class UpdateTracker:
    ...
    def start_update_tracking(
        self, total_cases: int, warehouse_columns: List[str]
    ) -> None:
    ...
    def log_case_update(
        ...
        force_change_type: str = None,
        force_priority: str = None,
    ) -> None:
    ...
    def generate_change_comparison_report(self) -> Dict[str, Any]:
        ...
        if self.changes:
            ...
            summary["counts"]["by_priority"] = {
                str(k): int(v) for k, v in by_prio.items()
            }
        return summary
