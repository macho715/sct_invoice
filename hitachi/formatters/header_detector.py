...
class HeaderDetector:
    ...
    def __init__(
        self, threshold: float = 78.0, custom_synonyms: Dict[str, List[str]] = None
    ):
        """
        초기화
    ...
    def detect_header_row(self, df: pd.DataFrame) -> Optional[int]:
        ...
        for row_idx in range(min(10, len(df))):  # 처음 10행만 검사
            try:
                ...
                print(f"[DEBUG] 행 {row_idx} 처리 오류: {e}")
                continue
            ...
            try:
                ...
                print(f"[DEBUG] 헤더 점수 계산 오류 (행 {row_idx}): {e}")
                continue
    ...
    def _calculate_confidence(self, row_values: List[str], score: int) -> float:
        ...
        # 비어있지 않은 값들의 개수
        non_empty_count = len(
            [val for val in row_values if val and str(val).strip() != "nan"]
        )
    ...
    def extract_column_names(self, df: pd.DataFrame, header_row: int) -> List[str]:
        ...
        header_values = df.iloc[header_row].astype(str).values
        return [
            val if val != "nan" else f"Column_{i}"
    ...
    def _calculate_header_score(self, row_values: List[str]) -> int:
        ...
        # 제목 행으로 보이는 패턴 감점
        if any(
            title_word in row_text
            for title_word in ["PROJECT", "MASTER FILE", "WAREHOUSE DATA"]
        ):
            return -1

        # 실제 데이터 행 패턴 감점 - 더 정교한 로직
        non_null_values = [
            val for val in row_values if val is not None and str(val) != "nan"
        ]
        if len(non_null_values) >= 3:  # 충분한 데이터가 있는 경우에만 체크
            # 첫 번째 값이 숫자이고 나머지가 HVDC 코드 패턴이면 데이터 행
            if (
                str(non_null_values[0]).isdigit()
                and len(non_null_values) > 1
                and "HVDC-ADOPT" in str(non_null_values[1])
            ):
                return -1
