...
def region_of(place: str, region_rules: dict | None = None) -> str:
    ...
    if "MUSSAFAH" in p or "ICAD" in p or "MARKAZ" in p or "M44" in p:
        return "MUSSAFAH"
    if "MINA" in p or "FREEPORT" in p or "ZAYED" in p or "JDN" in p or "PORT" in p:
        return "MINA"
    if "MIRFA" in p or "PMO" in p:
        return "MIRFA"
    if "SHUWEIHAT" in p or "S2" in p or "S3" in p or "POWER" in p:
        return "SHUWEIHAT"
    return "OTHER"
...
def build_baseline_from_approved(approved_df: pd.DataFrame) -> pd.DataFrame:
    df = (approved_df or pd.DataFrame()).copy()
    for c in [
        "origin",
        "destination",
        "vehicle",
...
def find_best_ref(row, baseline_df, cfg) -> Tuple[float, dict]:
    ...
    if cfg.get("fallbacks", {}).get("use_region_pool", True) and not baseline_df.empty:
        ro = region_of(row["origin_norm"], cfg.get("region_rules"))
        rd = region_of(row["destination_norm"], cfg.get("region_rules"))
        pool = baseline_df.copy()
        if "region_o" in pool.columns and "region_d" in pool.columns:
            pool = pool[(pool["region_o"] == ro) & (pool["region_d"] == rd)]
        pool = pool[
            (pool["vehicle"] == row["vehicle"])
            & (pool["unit"] == row.get("unit", "per truck"))
        ]
        if not pool.empty:
...
def build_special_set(ledger_df: pd.DataFrame) -> set:
    ...
    on = cols.get("origin_norm", cols.get("origin"))
    dn = cols.get("destination_norm", cols.get("destination"))
    v = cols.get("vehicle")
    u = cols.get("unit")
    special = set()
    if on and dn and v:
...
def validate_domestic(
    ...
    df = invoice_df.copy()
    # normalize numeric
    for col in ["rate_usd", "distance_km", "quantity", "qty"]:
        if col in df.columns:
    ...
    df = canonicalize_od(
        df.rename(
            columns={
                "place_loading": "origin",
                "place of loading": "origin",
                "place_delivery": "destination",
    ...
    # if Approved empty → derive from dataset
    if approved is None or approved.empty:
        temp = df.copy()
        for c in ["rate_usd", "distance_km"]:
            temp[c] = pd.to_numeric(temp.get(c), errors="coerce")
        grp_cols = ["origin_norm", "destination_norm", "vehicle", "unit"]
        derived = (
            temp.groupby(grp_cols, dropna=False)
            .agg(
        ...
        derived = derived.rename(
            columns={"origin_norm": "origin", "destination_norm": "destination"}
        )
        baseline = build_baseline_from_approved(derived)
    ...
    # adjusters (HAZMAT/CICPA 등)
    adj = cfg.get("adjusters", {})
    if adj.get("enabled", False):
        ...
        def _apply_adj(row):
            ...
            if pd.isna(ref):
                return ref
            v = str(row.get("vehicle", "")).upper()
            mul = 1.0
            for r in rules:
                key = str(r.get("if_vehicle_contains", "")).upper()
                if key and key in v:
                    mul *= float(r.get("rate_multiplier", 1.0))
            return round(ref * mul, 2)

        df["ref_rate_usd"] = df.apply(_apply_adj, axis=1)
    ...
    def _special(r):
        k = build_special_key(
            ...
            r["destination_norm"],
            r.get("vehicle", ""),
            r.get("unit", "per truck"),
        )
        return cfg["special_zone"]["label"] if k in special_set else None

    df["special_status"] = df.apply(_special, axis=1)
    ...
    # confidence
    # similarity proxy: 1 for direct, else 0.6 for similarity, else 0.4/0.3…
    def _sim_score(m):
        if m == "direct":
            return 1.0
        if m == "similarity":
    ...
    sim = df["ref_method"].apply(_sim_score)
    c_delta = 1.0 - np.minimum(df["delta_pct"].fillna(30.0), 30.0) / 30.0
    completeness = (
        df[["origin_norm", "destination_norm", "vehicle", "unit", "rate_usd"]]
        .notna()
        .mean(axis=1)
    )
    df["confidence"] = (0.5 * sim + 0.3 * c_delta + 0.2 * completeness).clip(0, 1)
    ...
    def _risk(row):
        ...
        return round(delta_norm * w["delta_weight"] + anomaly * w["anomaly_weight"], 3)
    ...
    def decide(row):
        ...
        # UNDER(과소) CRITICAL은 보류로 완충
        if band == "CRITICAL" and row.get("delta_pct", 0) < 0:
            return "PENDING_REVIEW"
        ...
        if band == "HIGH":
            return "PENDING_REVIEW"
        if row.get("fixed_cost_suspect", 0) == 1 or row.get("anomaly", 0) == 1:
            return "PENDING_REVIEW"
        # high-confidence gate
        if band in ["PASS", "WARN"] and (
            row.get("confidence", 0) >= 0.92
            or row.get("ref_method") in ["direct", "similarity"]
        ):
            return "VERIFIED"
    ...
    # alias suggestions
    def suggest_aliases(
        inv_df: pd.DataFrame, norm_map: dict, cfg: dict
    ) -> pd.DataFrame:
    ...
    alias_df = (
        suggest_aliases(df, norm_map, cfg)
    ...
    recap = [
        ...
        "I:: {sources:['invoice','executed_ledger','mapping']}",
        "S:: plan→normalize→join(exact→sim≥0.60)→score→export",
        "M:: {report.xlsx, recap.card, proof.artifact.json}",
    ]
    artifact = {
        ...
        "alias_suggestions": alias_df.head(200).to_dict(orient="records"),
    }
